...
var payouts = {
	"brell": [0, 0],
	"brell-inverted": [0, 0.5],
	"color-inverted-brell": [0.5, 1],
	"color-inverted-brell-inverted": [1, 1.5],
	"omega-brell": [2, 2.5],
	"omega": [2.5, 3],
	"whoop": [3, 3.5],
	"mad-brell": [3.5, 4],
...
function spin() {
	...
	if (bet === null) {
		updateDisplay("Please select a bet amount!", null);
		setTimeout(function() {
	...
	setTimeout(function() {
		...
		for (var key in counts) {
			if (counts[key] === 3) payout = bet * payouts[key][1];
			else if (counts[key] === 2) payout = bet * payouts[key][0];
		}
